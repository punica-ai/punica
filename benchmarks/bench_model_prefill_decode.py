...
class model_Resources:
  ...
  def __init__(
    ...
    self.input_ids = torch.randint(
        0,
        32000, (seqlen * prefills + decodes,),
        dtype=torch.int64,
        device=device)
    self.prefill_kv = BatchedKvCache([
        KvCache(self.kvpool, seqlen) for _ in range(prefills)
    ]) if prefills else None
    self.decode_kv = BatchedKvCache([
        KvCache(self.kvpool, seqlen) for _ in range(decodes)
...
def bench_model_prefill_decode(f):
  ...
  all_ = list(
      itertools.product(
          zip(num_heads_, num_layers_, intermediate_size_), prefill_decode_,
          seqlen_, batch_size_))
  last_num_heads = 0
  model = None
  for ((num_heads, num_layers, intermediate_size), (prefill, decode), seqlen,
