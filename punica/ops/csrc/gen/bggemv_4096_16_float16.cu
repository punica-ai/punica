#include <cuda_fp16.h>
extern "C" __global__ void __launch_bounds__(256) bggemv_4096_16_float16_kernel(int64_t* __restrict__ I, half* __restrict__ W, half* __restrict__ X, half* __restrict__ Y, int64_t B, int64_t layer_idx, int64_t num_layers, int64_t num_loras) {
  half Y_rf_local[1];
  __shared__ half red_buf0[256];
  Y_rf_local[0] = __float2half_rn(0.000000e+00f);
  for (int64_t ax0_fused_0 = 0; ax0_fused_0 < 16L; ++ax0_fused_0) {
    Y_rf_local[0] = (Y_rf_local[0] + (X[(((blockIdx.y * 4096L) + (ax0_fused_0 * 256L)) + threadIdx.x)] * W[((((((I[blockIdx.y] * num_layers) * 65536L) + (layer_idx * 65536L)) + (blockIdx.x * 4096L)) + (ax0_fused_0 * 256L)) + threadIdx.x)]));
  }
  __syncthreads();
...
extern "C" void launch_bggemv_4096_16_float16_kernel(void* __restrict__ I, void* __restrict__ W, void* __restrict__ X, void* __restrict__ Y, int64_t B, int64_t layer_idx, int64_t num_layers, int64_t num_loras) {
  dim3 grid(16, B);
  dim3 block(256);
  bggemv_4096_16_float16_kernel<<<grid, block>>>((int64_t*)I, (half*)W, (half*)X, (half*)Y, B, layer_idx, num_layers, num_loras);
}