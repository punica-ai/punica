...
extern "C" __global__ void __launch_bounds__(32) rotary_mha_decode_kvconst_56_128_48_2048_float16_kernel(half* __restrict__ K_proj, half* __restrict__ O, half* __restrict__ Q_proj, half* __restrict__ V_proj, half* __restrict__ kvbuf, int64_t* __restrict__ kvidx, int64_t* __restrict__ past_len, int64_t B, int64_t layer_idx, int64_t nnz) {
  ...
  for (int64_t k_0 = 0; k_0 < 4L; ++k_0) {
    ...
    Q_rotary[((k_0 * 32L) + threadIdx.x)] = ((half)((((float)Q_proj[((((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (k_0 * 32L)) + threadIdx.x)]) * cos) + (((float)((2L <= k_0) ? Q_proj[(((((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (k_0 * 32L)) + threadIdx.x) - 64L)] : (Q_proj[(((((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (k_0 * 32L)) + threadIdx.x) + 64L)] * __float2half_rn(-1.000000e+00f)))) * sin)));
    K_last_rotary[((k_0 * 32L) + threadIdx.x)] = ((half)((((float)K_proj[((((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (k_0 * 32L)) + threadIdx.x)]) * cos) + (((float)((2L <= k_0) ? K_proj[(((((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (k_0 * 32L)) + threadIdx.x) - 64L)] : (K_proj[(((((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (k_0 * 32L)) + threadIdx.x) + 64L)] * __float2half_rn(-1.000000e+00f)))) * sin)));
  }
  __syncthreads();
  *(uint2*)(kvbuf + (((((kvidx[blockIdx.x] * 1409286144L) + (layer_idx * 29360128L)) + (past_len[blockIdx.x] * 7168L)) + (blockIdx.y * 128L)) + (threadIdx.x * 4L))) = *(uint2*)(K_last_rotary + (threadIdx.x * 4L));
  *(uint2*)(kvbuf + ((((((kvidx[blockIdx.x] * 1409286144L) + (layer_idx * 29360128L)) + (past_len[blockIdx.x] * 7168L)) + (blockIdx.y * 128L)) + (threadIdx.x * 4L)) + 14680064L)) = *(uint2*)(V_proj + (((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (threadIdx.x * 4L)));
  ...
  for (int64_t jo = 0; jo < ((past_len[blockIdx.x] + 31L) >> 5L); ++jo) {
    ...
    for (int64_t ji_k_fused_0 = 0; ji_k_fused_0 < 32L; ++ji_k_fused_0) {
      *(uint2*)(K_shared + ((ji_k_fused_0 * 128L) + (threadIdx.x * 4L))) = ((((jo * 32L) + ji_k_fused_0) <= past_len[blockIdx.x]) ? *(uint2*)(kvbuf + ((((((kvidx[blockIdx.x] * 1409286144L) + (layer_idx * 29360128L)) + (jo * 229376L)) + (ji_k_fused_0 * 7168L)) + (blockIdx.y * 128L)) + (threadIdx.x * 4L))) : make_uint2(__pack_half2(__float2half_rn(0.000000e+00f), __float2half_rn(0.000000e+00f)), __pack_half2(__float2half_rn(0.000000e+00f), __float2half_rn(0.000000e+00f))));
    }
    __syncthreads();
    #pragma unroll
    ...
    for (int64_t ji_k_fused_0_1 = 0; ji_k_fused_0_1 < 32L; ++ji_k_fused_0_1) {
      *(uint2*)(K_shared + ((ji_k_fused_0_1 * 128L) + (threadIdx.x * 4L))) = ((((jo * 32L) + ji_k_fused_0_1) <= past_len[blockIdx.x]) ? *(uint2*)(kvbuf + (((((((kvidx[blockIdx.x] * 1409286144L) + (layer_idx * 29360128L)) + (jo * 229376L)) + (ji_k_fused_0_1 * 7168L)) + (blockIdx.y * 128L)) + (threadIdx.x * 4L)) + 14680064L)) : make_uint2(__pack_half2(__float2half_rn(0.000000e+00f), __float2half_rn(0.000000e+00f)), __pack_half2(__float2half_rn(0.000000e+00f), __float2half_rn(0.000000e+00f))));
    }
    uint mask_1[1];
    float t0_1[1];
  ...
  *(uint2*)(O + (((blockIdx.x * 7168L) + (blockIdx.y * 128L)) + (threadIdx.x * 4L))) = __1;
}
...
extern "C" void launch_rotary_mha_decode_kvconst_56_128_48_2048_float16_kernel(void* __restrict__ K_proj, void* __restrict__ O, void* __restrict__ Q_proj, void* __restrict__ V_proj, void* __restrict__ kvbuf, void* __restrict__ kvidx, void* __restrict__ past_len, int64_t B, int64_t layer_idx, int64_t nnz) {
  dim3 grid(B, 56);
  dim3 block(32);
  rotary_mha_decode_kvconst_56_128_48_2048_float16_kernel<<<grid, block>>>((half*)K_proj, (half*)O, (half*)Q_proj, (half*)V_proj, (half*)kvbuf, (int64_t*)kvidx, (int64_t*)past_len, B, layer_idx, nnz);
}