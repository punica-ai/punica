...
__global__ void sgmv_shrink(T* y, T* x, T** w, IdType* s,
  ...
  for (uint32_t i = 0; i < (s_end - s_start + (num_warps * block_m - 1)) /
                               (num_warps * block_m);
       ++i) {
#pragma unroll
...
#pragma unroll
    for (uint32_t iter = 0; iter < num_stages; ++iter) {
      ...
      x_smem[iter].offset = smem_t::get_permuted_offset<num_cells_per_block_k>(
          ty * block_m + tx / 4, tx % 4);
...
#pragma unroll
      ...
      T* w_ptr = w[problem_id] + (ty * 16 + tx / 4) * d_in +
                 (2 * num_k_frags * iter + tx % 4) * cell_capacity<T>();
      w_smem[iter].offset = smem_t::get_permuted_offset<num_cells_per_block_k>(
          ty * 16 + tx / 4, tx % 4);
...
#pragma unroll
    for (uint32_t iter = 0; iter < num_iterations; ++iter) {
      ...
      x_smem[stage_idx].offset =
          smem_t::get_permuted_offset<num_cells_per_block_k>(
              ty * block_m + tx % 16, tx / 16);
#pragma unroll
      for (uint32_t fk = 0; fk < num_k_frags; ++fk) {
        x_smem[stage_idx].ldmatrix_m8n8x4(x_frag[fk]);
        x_smem[stage_idx].offset =
            (x_smem[stage_idx].offset ^ 0x2) + (fk & 0x1) * 8;
      }
...
#pragma unroll
        for (uint32_t fk = 0; fk < num_k_frags; ++fk) {
          ...
          w_smem[stage_idx].offset =
              (w_smem[stage_idx].offset ^ 0x2) + (fk & 0x1) * 8;
        }
        w_smem[stage_idx].offset +=
            16 * num_cells_per_block_k - 4 * num_k_frags;
      }
...
#pragma unroll
          ...
          x_ptr += 8 * d_in - 2 * cell_capacity<T>() * num_k_frags;
          x_smem[stage_idx].offset +=
              8 * num_cells_per_block_k - 4 * num_k_frags;
        }
        row_idx -= 8;
...
#pragma unroll
    ...
    // store y_frag
    constexpr uint32_t num_cells_per_d_out =
        (d_out < 32 ? 32 : d_out) / cell_capacity<T>();
    y_smem.offset = smem_t::get_permuted_offset<num_cells_per_d_out>(
        ty * block_m + tx / 4, 0);
#pragma unroll
    for (uint32_t fn = 0; fn < num_warps; ++fn) {
      vec_cast<T, float, 2>((T*)(y_smem.base + y_smem.offset) + (tx % 4) * 2,
                            &y_frag[fn][0]);
      vec_cast<T, float, 2>(
          (T*)(y_smem.base + y_smem.offset + 8 * num_cells_per_d_out) +
              (tx % 4) * 2,
          &y_frag[fn][2]);
      vec_cast<T, float, 2>(
          (T*)(y_smem.base + (y_smem.offset ^ 0x1)) + (tx % 4) * 2,
          &y_frag[fn][4]);
      vec_cast<T, float, 2>(
          (T*)(y_smem.base + (y_smem.offset ^ 0x1) + 8 * num_cells_per_d_out) +
              (tx % 4) * 2,
          &y_frag[fn][6]);
    ...
    // store y
    uint32_t row_idx = s_start + (i * num_warps + ty) * 16 + tx % 16;
    T* y_ptr = y + row_idx * d_out + (tx / 16) * cell_capacity<T>();
    y_smem.offset = smem_t::get_permuted_offset<num_cells_per_d_out>(
        ty * block_m + tx % 16, tx / 16);
    for (uint32_t fn = 0; fn < num_warps; ++fn) {
      if (row_idx < s_end) {
      ...
      y_smem.offset = (y_smem.offset ^ 0x2) + 8 * (fn & 0x1);
    }
  }
}
